/* P2 polish layer is loaded on every page that uses the shared script. */
(() => {
...
const initParticleNetwork = () => {
  ...
  if (!canvas || !hero || reduceMotion || isTouchDevice) return;
...
const initTextReveals = () => {
  ...
  if (heroHeading && window.innerWidth > 800 && !reduceMotion) {
    ...
    setTimeout(() => heroHeading.classList.add('typing-done'), 1800);
  } else if (heroHeading) {
